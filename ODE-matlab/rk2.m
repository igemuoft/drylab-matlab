function xc=rk2(fs,x0,y,tc);
% function xc=rk2(fs,x0,y,tc);
%
% (c) 2001, Peter Brinkmann (brinkman@math.uiuc.edu)
%   Adapted for a specific purpose by andrew West for iGEM Toronto (2017)
%
...
%   y:  the second value for the inline function. In the case of this
%       function, it is currently meant for a vector.
...
% the Free Software Foundation (http://www.gnu.org/copyleft/gpl.html).
	...
	for i=1:(length(tc)-1)
        x2 = y(i);
		h=tc(i+1)-tc(i);
		k1=feval(fs,tc(i),x, x2);
		k2=feval(fs,tc(i)+h/2,x+k1*h/2, x2);
		k3=feval(fs,tc(i)+h/2,x+k2*h/2, x2);
		k4=feval(fs,tc(i)+h,x+k3*h, x2);
